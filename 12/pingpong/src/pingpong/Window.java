...
public class Window extends JFrame implements KeyListener 
{
	...
	public void keyPressed(KeyEvent e) { //按下鍵盤
		racquet.KeyPressed(e);
	}
	public void keyReleased (KeyEvent e) { //放開鍵盤
		racquet.KeyReleased (e);
	}
	public void keyTyped(KeyEvent e) { //按下及放開間的動作
	...
	ball.moveBall();
    racquet.moveRacquet();
	int paintAreaSize=Ball.getBallsize()+Math.max(Math.abs(ball.incx),Math.abs(ball.incy));
	repaint(ball.x-paintAreaSize,ball.y-paintAreaSize,paintAreaSize*2,paintAreaSize*2);
	}
	
	
	public static void main(String[] args) throws InterruptedException // 這個是丟擲異常用來防止執行緒異常的
	{
		Window jun = new Window(); // 建立window物件
       while (true){
    	   jun.move();
    	   jun.repaint();
    	   Thread.sleep(60);
